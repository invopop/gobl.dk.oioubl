...
func payInstructionsRules() *rules.Set {
	return rules.For(new(pay.Instructions),
		...
		rules.Assert("07", "a domestic bank transfer (payment-means 42) requires the bank registration number (up to 4 digits) as the credit-transfer name (F-LIB124 / F-LIB130)",
		...
		rules.Assert("09", "NemKonto (payment-means 97) must not carry a payment reference: PaymentID is not allowed (F-LIB161)",
			is.Func("nemkonto has no payment reference", nemKontoHasNoRef)),
		rules.Assert("10", "NemKonto (payment-means 97) must not carry a direct debit: PayerFinancialAccount is not allowed (F-LIB163)",
			is.Func("nemkonto has no direct debit", nemKontoHasNoDirectDebit)),
	)
}
...
// dkBankRegNrValid checks the bank registration number (1-4 digits), which a
// domestic transfer carries as the credit transfer's name (F-LIB124/F-LIB130).
func dkBankRegNrValid(val any) bool {
	...
	ct := firstCreditTransfer(instr)
	return ct != nil && isNumericOfLen(ct.Name, 1, 4)
}
...
func nemKontoHasNoRef(val any) bool {
	instr, ok := val.(*pay.Instructions)
	if !ok || instr == nil || instr.Ext.Get(untdid.ExtKeyPaymentMeans) != "97" {
		return true
	}
...
func nemKontoHasNoDirectDebit(val any) bool {
	instr, ok := val.(*pay.Instructions)
	if !ok || instr == nil || instr.Ext.Get(untdid.ExtKeyPaymentMeans) != "97" {
		return true
	}
	return instr.DirectDebit == nil
}
...
func isNumericOfLen(s string, minLen, maxLen int) bool {
	return is.Length(minLen, maxLen).Check(s) && is.Digit.Check(s)
}
