	...
	return rules.For(new(bill.Payment),
		rules.When(bill.PaymentTypeIn(bill.PaymentTypeRequest),
			...
			rules.Field("supplier",
				rules.Assert("03", "supplier must have an ISO 6523 endpoint or inbox (F-REM018)",
			...
			rules.Field("customer",
				...
				rules.Assert("06", "customer must have an ISO 6523 endpoint or inbox (F-REM025)",
					is.Func("has endpoint or inbox", partyHasEndpointOrInbox)),
			...
			rules.Field("payee",
				rules.AssertIfPresent("09", "payee must have an ISO 6523 endpoint or inbox (F-REM034)",
					is.Func("has endpoint or inbox", partyHasEndpointOrInbox)),
			),
			rules.Field("methods",
				rules.Each(
					rules.When(is.Func("fik kortart 73 carrying a payment reference", fik73RecordWithReference),
						rules.Assert("10", "FIK payment id 73 must not carry a payment reference, OIOUBL has no element for it (F-LIB275)", is.Func("never", neverTrue)),
					),
				),
			),
		),
