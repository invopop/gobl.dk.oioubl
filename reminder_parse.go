...
// Convert turns a parsed OIOUBL Reminder into a GOBL envelope wrapping a
// bill.Payment of type "request".
func (rem *Reminder) Convert() (*gobl.Envelope, error) {
	pmt, err := rem.goblPayment()
	if err != nil {
		return nil, err
	}
...
func (rem *Reminder) goblPayment() (*bill.Payment, error) {
	out := &bill.Payment{
		...
		Supplier: goblParty(rem.AccountingSupplierParty.Party),
		Customer: goblParty(rem.AccountingCustomerParty.Party),
	}
	if rem.PayeeParty != nil {
		out.Payee = goblParty(rem.PayeeParty)
	}
	...
	for i := range rem.PaymentMeans {
		if m := goblReminderMethod(&rem.PaymentMeans[i]); m != nil {
			out.Methods = append(out.Methods, m)
		}
	}

	applyReminderParse(out, rem)
...
func goblReminderMethod(pm *PaymentMeans) *pay.Record {
	channel := ""
	if pm.PaymentChannelCode != nil {
		channel = pm.PaymentChannelCode.Value
	}
	if channel == paymentChannelGiro || channel == paymentChannelFIK {
...
// applyReminderParse restores the reminder type and sequence onto the payment
// so they round-trip: the type is a document-variant tag (Advis → the advis
// tag; Reminder is the untagged default), the sequence a payment extension.
func applyReminderParse(pmt *bill.Payment, rem *Reminder) {
