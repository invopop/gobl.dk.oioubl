...
// convertInvoiceFrom converts a GOBL fixture from test/data/convert.
func convertInvoiceFrom(t *testing.T, name string) *dkoioubl.Invoice {
	t.Helper()
	env := loadTestEnvelope(t, filepath.Join(getConvertPath(), name))
...
func TestDualCurrencyTotals(t *testing.T) {
	...
	// ProfileID carries OIOUBL scheme attributes natively.
...
// TestMixedCategoryExchangeRate locks the exchange-rate reconstruction for a
// foreign-currency invoice that mixes StandardRated and ZeroRated lines.
...
func TestMixedCategoryExchangeRate(t *testing.T) {
	doc, err := dkoioubl.Parse([]byte(mixedCategoryDualCurrencyXML))
	require.NoError(t, err)
	inv, ok := doc.(*dkoioubl.Invoice)
	require.True(t, ok)
